 ...
 */
 ...
 var  ayant_un_attribut_next = new Array("#pointWorld", "#add_button", "#possessAttributValueWord", "#virguleWord"); 
 
 var de_type_next = new Array("#pointWorld","#possessAttributeCondWord", "#typeCond", "#add_button") ; 
 
 var possessAttributValueWord_next = new Array("#pointWorld","#possessAttributeCondWord", "#add_button", "#possessAttributValueWord");
	   ...
	   if( querylist[k][0] == focus ) 
	   {
	      if( querylist[k].length <= 2 )
		  {
			...
			else
			{
				active_next([]);
				/* POUR LE MODULE */
				/* Reactivation des éléments pour ajouter une nouvelle requête */
				$(".jecherche").button( "option", "disabled", false );
				$(".soit").button( "option", "disabled", false );
				$(".jenomme").button( "option", "disabled", false );
				$(".parmi").button( "option", "disabled", false );
			}
	  }
		  else
	      {
...
*/
 function newlist( debut, next_list) 
 {
	...
	/* POUR LE MODULE */
	/* Desactivation des boutons pour ajouter un nouvelle liste */
	$(".jecherche").button( "option", "disabled", true );
	$(".soit").button( "option", "disabled", true );
	$(".jenomme").button( "option", "disabled", true );
	$(".parmi").button( "option", "disabled", true );
